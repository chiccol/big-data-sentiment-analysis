tp_test="diff_companies_test_dataset.json"
tp_train="training_dataset.json"
yt_test="test_dataset.json"
yt_train="train_dataset_balanced.json"

# Check if the files already exist
if [ ! -f "$tp_test" ]; then
    echo "Downloading Trustpilot test..."
    gdown https://drive.google.com/uc?id=1wPJC00XvvTJ0wqQry0EmDG0hRmy7-pcd
else
    echo "Trustpilot test already exists."
fi

if [ ! -f "$tp_train" ]; then
    echo "Downloading Trustpilot train..."
    gdown https://drive.google.com/uc?id=1oXOdq8x4NuqAfbaR2ak8n4atclAlHxdz
else
    echo "Trustpilot train already exists."
fi

if [ ! -f "$yt_test" ]; then
    echo "Downloading YouTube test..."
    gdown https://drive.google.com/uc?id=1ij9G4y2MqRuP1-s0KEcBSwGtimWz6uyf
else
    echo "YouTube test already exists."
fi

if [ ! -f "$yt_train" ]; then
    echo "Downloading YouTube train..."
    gdown https://drive.google.com/uc?id=13OCC_WsLB2Vwt3pz2nm_hEFjNszfLV15
else
    echo "YouTube train already exists."
fi

python3 main.py